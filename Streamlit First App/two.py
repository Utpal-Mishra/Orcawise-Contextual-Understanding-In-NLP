...
def app():
    ...
    total_data_bigram = get_imp(bow=[content], mf=5000, ngram=2)
    total_data_bigram = pd.DataFrame(total_data_bigram).reset_index()
    total_data_bigram.columns = ['entity', 'occurance']
    total_data_bigram = total_data_bigram[total_data_bigram['occurance'] > 1]
    
    """if st.checkbox("Show BI-GRAM DataFrame"):
        st.dataframe(total_data_bigram)"""
    
    
    dictionary_bigram = dict()
    for entities_bigram, occurances in zip(total_data_bigram['entity'], total_data_bigram['occurance']):
        # this condition will check if digits and spaces are only there in string
        if re.match("^[0-9 ]+$", entities_bigram):
            continue
        dictionary_bigram[entities_bigram] = occurances

    df = pd.DataFrame(dictionary_bigram, index = [0])
